...
3. **Replace the `_TBD_`s** in two places:
   - **`00N…/README.md`:** Runtime, Full show notes URL, Apple link, YouTube link.
   - **Root `README.md` row:** the runtime after the date, `Watch`, `Apple`, `Show notes`, and swap the `<i>thumbnail TBD</i>` placeholder for the YouTube thumbnail image (same `<a><img …hqdefault.jpg></a>` form the other rows use).

4. **Confirm before commit.** Show `git diff` and verify `grep -c _TBD_ 00N…/README.md` is `0` (or that any remaining `_TBD_` is genuinely still unavailable). On approval, commit straight to `main`.
